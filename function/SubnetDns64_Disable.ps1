function Disable-SubnetDns64
{
    [Alias('subnet_dns64_dis')]
    ...
    PROCESS
    {
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Disable DNS64'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -Verbose:$false -EnableDns64 $false $_.SubnetId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

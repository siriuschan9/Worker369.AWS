$Script:DefaultEC2StatusAlarmConfig = @{
    Expression = 'Minimum(StatusCheckFailed) > 0 ; 60 ; 1/1 ; breaching'
...
.EXAMPLE
Get-EC2Instance -Select Reservations.Instances | New-Ec2StatusCheckAlarm -Force -WarningAction 'Silently Continue'

This example creates/overwrite all status check alarm on all EC2 instances,
with the default settings, suppressing all warnings.

.EXAMPLE
New-Ec2StatusCheckAlarm 'i-01234567890abcdef' 'Minimum(StatusCheckFailed) > 0 ; 300 ; 3/3 ; breaching'

This exmaple creates a status check alarm on the EC2 instance 'i-01234567890abcdef'
that triggers after 3 consecutive failed check on 5-min datapoints.
...
function New-EC2StatusAlarm
{
    ...
    PROCESS
    {
        ...
        # Figure out the alarm name first so that we can check if the same alarm already exists.
        $_alarm_name = "EC2 Status Check Alarm ($($_account_name)/$($_instance_name))"
        ...
        $_params = @{
            ...
            AlarmDescription = (
                "EC2 Status Check Alarm for {0}. Triggers when {1} for {2}-out-of-{3} {4} datapoints" -f `
                $_instance_name,
        ...
        $_target    = "$($_alarm_name)"
        $_operation = "Creating new EC2 status alarm"
        ...
        try{
            ...
            Write-Message -Output (
                "|- A new EC2 status alarm named `"$($_alarm_name)`" has been created for `"$($_format_ec2)`"."
            )
        }
        catch{
            # Remove caught exception emitted into $Error list.

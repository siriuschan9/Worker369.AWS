<#
.SYNOPSIS
This cmdlet enables NAU Metrics for one for more VPC(s).
...
.EXAMPLE
Enable-VpcNauMetrics vpc-1234567890abcdef0

This example enables NAU Metrics for the VPC vpc-1234567890abcdef0.

.EXAMPLE
Enable-VpcNauMetrics -VpcName example-*

This example enables NAU Metrics for all VPCs that starts with "example-".
#>
function Enable-VpcNauMetrics
{
    [Alias('vpc_nau_en')]
    ...
    PROCESS
    {
        ...
        $_vpc_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_vpc, 'Enable NAU Metrics'))
            {
                # Use AWS CLI to call the API to enable the NAU for this VPC.
                try {
                    Edit-EC2VpcAttribute -Verbose:$false -EnableNetworkAddressUsageMetric $true $_.VpcId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

<#
.SYNOPSIS
This cmdlet removes a secondary IPv4 CIDR block from a VPC.
...
.PARAMETER Cidr
The -Cidr Parameter specifies the secondary IPv4 CIDR block to remove from the VPC.

.EXAMPLE
Remove-VpcIpv4Cidr vpc-1234567890abcdef0 10.255.0.0/16

This example removes the IPv4 CIDR block "10.255.0.0/16" from the VPC vpc-1234567890abcdef0.

.EXAMPLE
Remove-VpcIpv4Cidr -VpcName example-* -Ipv4Cidr 10.255.0.0/16

This example removes the secondary IPv4 CIDR block "10.255.0.0/16" from all VPCs with Name that starts with "example-".
#>
function Remove-VpcIpv4Cidr
{
    [Alias('vpc_ipv4_rm')]
    ...
    PROCESS
    {
        ...
        # Loop through each VPC to perform the CIDR removal.
        $_vpc_list | ForEach-Object {
            ...
            try {
                # Try to retrieve the association given the CIDR block.
                $_target_assoc = $_.CidrBlockAssociationSet | Where-Object {
                    $_.CidrBlock -eq $_cidr -and
                    $_.CidrBlockState.State -eq 'associated'
                }

                if ($_target_assoc)
                {
                    # Display the What-If/Confirmation prompt.
                    if ($PSCmdlet.ShouldProcess($_format_vpc, 'Remove IPv4 CIDR block $_cidr'))
                    {
                        # Disassociate the CIDR block.
                        $_result = Unregister-EC2VpcCidrBlock -Verbose:$false $_target_assoc.AssociationId

                        # Wait for the dissociation to finish.
                ...
                {
                    Write-Error "|- $_cidr is not an associated block for $_format_vpc."
                    return
                }
            }
            catch{

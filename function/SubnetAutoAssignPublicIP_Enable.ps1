function Enable-SubnetAutoAssignPublicIP
{
    [Alias('subnet_aip_en')]
    ...
    PROCESS
    {
       # Use snake_case.
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Enable Auto Assign Public IP'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -SubnetId $_.SubnetId -MapPublicIpOnLaunch $true -Verbose:$false
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

function Enable-SubnetAAAARecord
{
    [Alias('subnet_aaaa_en')]
    ...
    PROCESS
    {
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Enable Resource Name DNS AAAA Record on Launch'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -SubnetId $_.SubnetId -EnableResourceNameDnsAAAARecordOnLaunch $true -Verbose:$false
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

<#
.SYNOPSIS
This cmdlet enables DNS Hostnames for one for more VPC(s).
...
function Enable-VpcDnsHostnames
{
    ...
    PROCESS
    {
        ...
        $_vpc_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_vpc, 'Enable DNS Hostnames'))
            {
                # Call the API to enable the DNS hostnames for this VPC.
                try {
                    Edit-EC2VpcAttribute -EnableDnsHostname $true -Verbose:$false $_.VpcId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

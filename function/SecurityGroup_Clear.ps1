function Clear-SecurityGroup
{
    ...
    PROCESS
    {
        ...
        $_sg_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_sg, "Clear Security Group"))
            {
                $_all_rules      = $null
                $_inbound_rules  = $null
                $_outbound_rules = $null

                $_all_rules = Get-EC2SecurityGroupRule -Verbose:$false -Filter @{Name = 'group-id'; Values = $_.GroupId}

                $_inbound_rules  = $_all_rules | Where-Object -Not IsEgress
                $_outbound_rules = $_all_rules | Where-Object IsEgress

                try {
                    if ($_scope -in @('Inbound', 'All') -and $_inbound_rules) {
                        $_inbound_rules | Revoke-EC2SecurityGroupIngress -Verbose:$false $_.GroupId | Out-Null
                    }

                    if ($_scope -in @('Outbound', 'All') -and $_outbound_rules) {
                        $_outbound_rules | Revoke-EC2SecurityGroupEgress -Verbose:$false $_.GroupId | Out-Null
                    }
                }
                catch {
                    # Remove caught exception emitted into $Error list.

function Show-IdcAssignment
{
    ...
    $_raw = $_result_list | Select-Object $_select_list | Sort-Object $_sort_list

    $_users = $_raw | Group-Object User

    foreach ($_user in $_users)
    {
        Write-Host "$($_highlight)$($_user.Name)$($_reset)"

        $_accounts = $_user.Group | Group-Object Account

        $_width = $_accounts.Name | Measure-Object Length -Maximum | Select-Object -ExpandProperty Maximum
        foreach ($_account in $_accounts)
        {
            Write-Host -NoNewline ("|- {0, -$_width} : " -f $_account.Name)

            foreach ($_assignment in $_account.Group)
            {
                Write-Host -NoNewline "$($_assignment.PermissionSet)$($_dim):"

                $_groups = $_assignment.InheritedViaGroup -join '|'

                if ($_assignment.IsDirectlyAssigned.IsChecked) {
                    $_groups = ('_self_|' + $_groups).TrimEnd('|')
                }

                Write-Host -NoNewline ($_groups + " " + $_reset)
            }
            Write-Host $_reset
        }
        Write-Host

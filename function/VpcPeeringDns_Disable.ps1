function Disable-VpcPeeringDns
{
        ...
        if (-not $PSBoundParameters.ContainsKey('Left') -and -not $PSBoundParameters.ContainsKey('Right'))
        {
            $_error_record = New-ErrorRecord `
                -ErrorMessage (
                    ...
                    'Use -Left parameter to accept DNS on requester side from accepter side.' +
                    'Use -Right parameter to accept DNS on accepter side from requester side.'
                 ) `
    ...
    PROCESS {
        ...
        $_left     = -not $Left.IsPresent
        $_right    = -not $Right.IsPresent
        ...
        $_pcx_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_pcx, "Disable DNS resolution from remote VPC."))
            {

<#
.SYNOPSIS
This cmdlet disables DNS Resolution Support for one for more VPC(s).
...
function Disable-VpcDnsResolution
{
    ...
    PROCESS
    {
        ...
        $_vpc_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_vpc, 'Disable DNS Resolution'))
            {
                # Call the API to disable DNS resolution support for this VPC.
                try {
                    Edit-EC2VpcAttribute -EnableDnsSupport $false -Verbose:$false $_.VpcId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

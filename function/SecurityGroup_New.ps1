function New-SecurityGroup
{
    ...
    PROCESS
    {
        ...
        $_make_tags_with_name    = { New-TagSpecification -ResourceType security-group -Tag $_tag -Name $_name}
        $_make_tags_without_name = { New-TagSpecification -ResourceType security-group -Tag $_tag }
        ...
        if ($PSCmdlet.ShouldProcess($_format_vpc, 'Create New Security Group'))
        {
            # Invoke the API to create the new Security Group.
            try {
                $_new_sg_id = New-EC2SecurityGroup -Verbose:$false `
                    -TagSpecification $_tag_specification `
                    -GroupName $_group_name `
                    -Description $_desc `
                    -VpcId $_vpc.VpcId

                Write-Message -Output "|- GroupId: $_new_sg_id"
            }
            catch {
                # Remove caught exception emitted into $Error list.
                Pop-ErrorRecord $_

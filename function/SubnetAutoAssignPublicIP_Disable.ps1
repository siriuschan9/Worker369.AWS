function Disable-SubnetAutoAssignPublicIP
{
    [Alias('subnet_aip_dis')]
    ...
    PROCESS
    {
       # Use snake_case.
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Disable Auto Assing Public IP'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -SubnetId $_.SubnetId -MapPublicIpOnLaunch $false -Verbose:$false
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

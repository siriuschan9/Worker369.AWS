function Disable-SubnetAAAARecord
{
    [Alias('subnet_aaaa_dis')]
    ...
    PROCESS
    {
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Disable Resource Name DNS AAAA Record on Launch'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -SubnetId $_.SubnetId -EnableResourceNameDnsAAAARecordOnLaunch $false -Verbose:$false
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

function Disable-S3Versioning
{
    ...
    PROCESS
    {
        ...
        # Loop through each Bucket to disable Versioning.
        $_bucket_list | ForEach-Object {

            # Display What-If/Confirmation prompt.
            if ($PSCmdlet.ShouldProcess($_.BucketArn, 'Disable Object Versioning'))
            {
                # Edit the subnet's attribute.
                try {
                    Write-S3BucketVersioning `
                        -Verbose:$false -Region $_region -VersioningConfig_Status Suspended $_.BucketName
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

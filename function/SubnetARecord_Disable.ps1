function Disable-SubnetARecord
{
    [Alias('subnet_a_dis')]
    ...
    PROCESS
    {
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Disable Resource Name DNS A Record on Launch'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -Verbose:$false -EnableResourceNameDnsARecordOnLaunch $false $_.SubnetId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_

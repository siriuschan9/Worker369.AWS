function Enable-SubnetAutoAssignIpv6
{
    [Alias('subnet_aip6_en')]
    ...
    PROCESS
    {
        ...
        $_subnet_list | ForEach-Object {
            ...
            if ($PSCmdlet.ShouldProcess($_format_subnet, 'Enable Auto Assign IPv6 Address'))
            {
                # Edit the subnet's attribute.
                try {
                    Edit-EC2SubnetAttribute -Verbose:$false -AssignIpv6AddressOnCreation $true $_.SubnetId
                }
                catch {
                    # Remove caught exception emitted into $Error list.
                    Pop-ErrorRecord $_
